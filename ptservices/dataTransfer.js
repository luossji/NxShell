...
class NxDataTransferServer extends NxDataTransfer {
    ...
    isDirectoryEntry(entry) {
        ...
        if (entry.type === "dir" || entry.type === "directory" || entry.type === "d") {
            return true;
        }

        if (typeof entry.mode === "number") {
            return (entry.mode & 0xF000) === 0x4000;
        }
    ...
    getEntrySize(entry) {
        ...
        if (typeof entry.getSize === "function") {
    ...
    getEntryMTime(entry) {
        ...
        if (typeof entry.getMTime === "function") {
    ...
    async walkFolder(pathLib, fs, rootDir) {
        ...
        const walk = async (relPath) => {
            ...
            for (let i = 0; i < direntList.length; i++) {
                ...
                if (this.isDirectoryEntry(dirent)) {
                    folderList.push(dirpath);
                    isFolder = true;
                }

                const fileSize = isFolder ? 0 : this.getEntrySize(stats);
    ...
    async askIfNeed(sourcePathLib, sourceFS, destFS, sourcePath, destPath) {
        ...
        /* 源和目标的类型并不一致 */
        if ((this.isDirectoryEntry(sourceStat) && (!this.isDirectoryEntry(destStat))) || 
            ((!this.isDirectoryEntry(sourceStat)) && (this.isDirectoryEntry(destStat)))
        ) {
            this._emit({
                event: "error",
                args: {
        ...
        let question = this.isDirectoryEntry(sourceStat) ? "merge" : "overwrite";
        const basename = sourcePathLib.basename(sourcePath)

        const {action, keep} = await this._ask(question, {
            name: basename,
            dest: {
                lastModify: this.getEntryMTime(destStat),
                size: this.getEntrySize(destStat)
            },
            src: {
                lastModify: this.getEntryMTime(sourceStat),
                size: this.getEntrySize(sourceStat)
            }
        });
            ...
            try {
                ...
                if (sourceCreateFolder) {
                    ...
                    if (!exists) {
                        ...
                        // 目录项不是文件夹，抱歉直接报错
                        if (!this.isDirectoryEntry(destStat)) {
                            this._emit({
                                event: "error",
                                args: {
                                    message: "",
