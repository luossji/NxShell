<template>
	<div class="pt-vnc-view">
		<div v-show="showSessionToolbar" class="n-editor-toolbar">
			...
			<div class="n-editor-toolbar__actions">
				<el-tooltip content="保存" placement="bottom">
					<el-button type="text" icon="el-icon-check" :disabled="!hasPendingChanges" @click="save" />
				</el-tooltip>
		...
		<div ref="editorRef" class="screen" :style="screenStyle"></div>
	</div>
</template>
...
import { computed, getCurrentInstance, onBeforeUnmount, onMounted, ref, watchEffect } from "vue"
...
const showSessionToolbar = ref(!getProfile("xterm")?.hideSessionShortcutBar)
...
const hasPendingChanges = computed(() => {
	...
	return !!editor && editor.state.doc.toString() !== oldValue.value
})

const screenStyle = computed(() => ({
	fontSize: `${fontSize.value}px`,
	height: showSessionToolbar.value ? 'calc(100% - 40px)' : '100%'
}))

const handleToggleShortcutBarEvent = (payload) => {
	if (!payload || payload.sessionId !== props.sessionId) {
		return
	}
	if (typeof payload.visible === 'boolean') {
		showSessionToolbar.value = payload.visible
	} else {
		showSessionToolbar.value = !showSessionToolbar.value
	}
	setTimeout(() => {
		editorInstance.value?.requestMeasure?.()
	}, 0)
}
...
watchEffect(() => {
	...
	const editor = editorInstance.value
	if (!editor) {
		nxTabsStore.updateEditChange(false)
		searchCursor.value = null
		return
	}
	editor.dispatch({
	...
	// 监听数据是否有变化
	nxTabsStore.updateEditChange(editor.state.doc.toString() !== oldValue.value)
	searchCursor.value = new SearchCursor(editor.state.doc, searchKeyWords.value)
})
...
onMounted(async () => {
	EventBus.subscript('toggle-session-shortcut-bar', handleToggleShortcutBarEvent)
...
onBeforeUnmount(() => {
	EventBus.unsubscript('toggle-session-shortcut-bar', handleToggleShortcutBarEvent)
})
</script>
