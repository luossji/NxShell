<template xmlns=''>
	<div class='pt-file-view'>
		<pt-toolbar v-show='showSessionToolbar' :centerStyle="{ overflow: 'hidden', marginLeft: '10px', marginRight: '10px' }">
			<template slot='left'>
				<n-space :size='5'>
					<el-tooltip
						class='item'
		...
		/>
		<!-- 查看文件属性弹窗 -->
		<el-dialog :title="$t('home.fileview.prop-dialog.file-props')" width='560px' :visible.sync='filePropDialog.show'>
			<div class='file-prop-dialog'>
				<el-scrollbar style='height: 100%'>
					<el-descriptions
						class='file-prop-dialog__descriptions'
						:column='1'
						size='large'
						:colon='false'
					...
					>
						<el-descriptions-item :span='10'>
							...
							<div class='file-prop-dialog__value file-prop-dialog__value--title'>{{ filePropDialog.dirent.filename }}</div>
						</el-descriptions-item>
						<el-descriptions-item :label="$t('home.fileview.prop-dialog.type')" :span='10'>
							<div class='file-prop-dialog__value'>{{ filePropDialog.dirent.type }}</div>
						</el-descriptions-item>
						<el-descriptions-item :label="$t('home.fileview.prop-dialog.location')" :span='10'>
							<div class='file-prop-dialog__value file-prop-dialog__value--multiline'>{{ filePropDialog.dirent.location }}</div>
						</el-descriptions-item>
						<el-descriptions-item :label="$t('home.fileview.prop-dialog.size')" :span='10'>
							<div class='file-prop-dialog__value'>{{ filePropDialog.dirent.size | fileSize }}</div>
						</el-descriptions-item>
						<el-descriptions-item :label="$t('home.fileview.prop-dialog.modify-time')" :span='10'>
							<div class='file-prop-dialog__value file-prop-dialog__value--multiline'>{{ filePropDialog.dirent.mtime }}</div>
						</el-descriptions-item>
						<el-descriptions-item :label="$t('home.fileview.prop-dialog.user')" :span='10'>
							<div class='file-prop-dialog__value'>{{ filePropDialog.dirent.user }}</div>
						</el-descriptions-item>
						<el-descriptions-item :label="$t('home.fileview.prop-dialog.group')" :span='10'>
							<div class='file-prop-dialog__value'>{{ filePropDialog.dirent.group }}</div>
						</el-descriptions-item>
						<el-descriptions-item :label="$t('home.fileview.prop-dialog.perms')" :span='10'>
							<div class='file-prop-dialog__value'>{{ filePropDialog.dirent.perms }}</div>
						</el-descriptions-item>
					</el-descriptions>
				</el-scrollbar>
			</div>
...
function createEmptyFilePropDirent() {
	return {
		filename: '',
		fileicon: 'file',
		location: '',
...
function createVirtualDirectoryDirent(name = '/') {
...
export default {
	...
	data() {
		return {
			...
			filePropDialog: {
				show: false,
				dirent: createEmptyFilePropDirent()
			},
			dirCreateDialog: {
				show: false,
				dirname: ''
	...
	computed: {
		...
		gridViewStyle() {
			return {
				height: this.showSessionToolbar ? 'calc(100% - 72px)' : 'calc(100% - 32px)'
			}
		}
	},
	...
	methods: {
		...
		canShowParentDirectory(dirPath) {
			return path.resolve(dirPath || '/') !== '/'
		},

		formatDirentDate(dateValue) {
			return isValidDate(dateValue) ? this.$d(dateValue, 'long') : '-'
		},
		...
		async readdir(dirPath) {
			...
			if (this.canShowParentDirectory(dirPath)) {
				const parentDirent = createParentDirent()
				const { icon, type } = this.getIconAndType(parentDirent)
				List.unshift({
					name: '..',
					icon,
					iconType: 'svg',
			...
			this.fileItems = List.map((dirent) => {
				...
				return {
					...
					lastModify: this.$t('home.fileview.mainview.columns.fmt-time', [
						this.formatDirentDate(dirent.getMTime())
					]),
		...
		async handleContextMenu_Prop() {
			...
			if (this.contextMenu.fileItemIdx === -1) {
				...
				try {
					...
					dirent = stat
						? new Dirent(path.basename(this.currentPath) || '/', stat)
						: createVirtualDirectoryDirent(path.basename(this.currentPath) || '/')
					location = path.dirname(this.currentPath)
				} catch (e) {
					console.log('handleContextMenu_Prop error ', e)
					dirent = createVirtualDirectoryDirent(path.basename(this.currentPath) || '/')
					location = path.dirname(this.currentPath)
				}
			} else {
				//const fileItem = this.fileItems[this.contextMenu.fileItemIdx];
			...
			this.filePropDialog.dirent = {
				...
				size: dirent.getSize(),
				mtime: this.formatDirentDate(dirent.getMTime()),
				user: dirent.getUid(),
				group: dirent.getGid(),
				perms: dirent.getPermsString()
			}
		...
		handleCloseFilePropDialog() {
			...
			this.filePropDialog.dirent = createEmptyFilePropDirent()
		},
		...
		async deleteFiles() {
			try {
				const selectedItems = this.selectedItems
					.map((itemIdx) => this.fileList[itemIdx])
					.filter((item) => !item?.isParentDirectory)
				if (selectedItems.length === 0) {
					return
				}
				await this.$confirm(
					this.$t('home.fileview.confirm-dialogs.delete-many.message', [selectedItems.length]),
		...
		handleDelete(fileIdx) {
			...
			if (!fileItem || fileItem.isParentDirectory) {
				return
			}
			const direntPath = path.resolve(this.currentPath, fileItem.dirent.name)
			if (fileItem.dirent.isDirectory()) {
		...
		handleFileSort(key) {
			const isSameKey = this.sortKey === key
			this.sortBy = isSameKey ? !this.sortBy : true
			this.sortKey = key

			const direction = this.sortBy ? 1 : -1
			const compareString = (left, right) => String(left || '').localeCompare(String(right || ''), undefined, { numeric: true, sensitivity: 'base' })
			...
			const compareByKey = (a, b) => {
				if (key === 'size') {
					...
					return compareNumber(a.sizeByte, b.sizeByte)
				}

				if (key === 'lastModify') {
					return compareNumber(new Date(a.lastModify), new Date(b.lastModify))
				}
			...
			const sortItems = (items) => {
				return [...items].sort((a, b) => {
					if (a.isParentDirectory || b.isParentDirectory) {
						if (a.isParentDirectory && b.isParentDirectory) {
							return 0
						}
						return a.isParentDirectory ? -1 : 1
					}
					...
					const compared = compareByKey(a, b)
					if (compared !== 0) {
			...
			if (this.searchKeyWords) {
				this.searchResult = sortItems(this.searchResult)
			}
		}
	},
...
.file-prop-dialog {
  ...
  height: 400px;
	...
	::v-deep .file-prop-dialog__descriptions .el-descriptions-item__cell {
		vertical-align: top;
	}

	::v-deep .file-prop-dialog__descriptions .el-descriptions-item__label,
	::v-deep .file-prop-dialog__descriptions .el-descriptions-item__content {
		line-height: 1.5;
		padding-top: 6px;
		padding-bottom: 6px;
	}
