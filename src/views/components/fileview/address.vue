<template>
    <div class="pt-file-view-address" :class="{editable: isEditable}">
        ...
        <div class="container" ref="container" @mousewheel="handleMouseWheel" @click="handleEnableEditPath">
            <div v-if="!isEditable" ref="address" class="address-list" @click.stop @dblclick.stop="handleEnableEditPath">
                <div class="address-list-item host" @click.stop="handleEnableEditPath">
                ...
                <div v-for="(entry, idx) in parsedPath" :key="entry.entry + '/' + idx" class="address-list-item">
                    <span @click.stop="handleChangePath(idx)" @dblclick.stop="handleEnableEditPath">{{ entry.entry }}</span>
        ...
        <div class="address-actions">
            <span class="btn-action" title="编辑路径" @click.stop="handleEnableEditPath">
                <i class="el-icon-edit" />
            </span>
            <span class="btn-action" :title="$t('home.sessions-context-menu.copy')" @click.stop="handleCopyPath">
...
export default {
    ...
    methods: {
        ...
        detectAddressListWidth() {
            this.btnScroll.contentWidth = this.$refs.address ? this.$refs.address.getBoundingClientRect().width : 0
        },
        ...
        handleCopyPath() {
            powertools.clipboardWriteText(this.value)
            this.$message.success(this.$t('components.copy-success'))
        },
