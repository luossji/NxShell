	...
	>
		<el-form ref="sshSubFormRef" :model="sshSubForm" :rules="rules" class="n-session-ssh-container" label-position="top" label-width="80px">
			<div class="n-session-ssh-container__left">
				...
				<host-icon-field v-model="sshSubForm.system" />
				<el-form-item :label="t('home.profile.base.host-group.title')" prop="group">
					<el-select v-model="sshSubForm.group" clearable :placeholder="t('home.profile.base.host-group.placeholder')" style="width: 100%">
...
const saveOrUpdate = async () => {
	...
	if (isEdit.value) {
		await sessionManager.updateSessionConfig(sessionConfig.value, sessionName, Object.assign({}, sessionConfig.value.config, sshSubForm.value), "", sshSubForm.value.group)
	} else {
		// 创建会话配置
