...
export class SessionConfig extends EventEmitter {
    ...
    update(name, configParam, description, options = {}) {
...
class SessionManager extends EventEmitter {
    ...
    /** @type {Promise<void>} */
    sessionConfigSaveTask = Promise.resolve();
    ...
    async saveSessionConfigs() {
        ...
        this.sessionConfigSaveTask = this.sessionConfigSaveTask
            .catch(() => undefined)
            .then(() => Storage.save("SESSIONS", sessCfgObjects, true));

        await this.sessionConfigSaveTask;
    }
    ...
    findSessionConfigParent(targetCfg, parentNode = this.sessionConfigsRoot) {
        if (!targetCfg || !(parentNode instanceof SessionConfig) || parentNode.type === SESSION_CONFIG_TYPE.NODE) {
            return null;
        }

        for (const childNode of parentNode.subSessions) {
            if (childNode === targetCfg || childNode._id === targetCfg._id || (targetCfg.uuid && childNode.uuid === targetCfg.uuid)) {
                return parentNode;
            }

            if (childNode.type === SESSION_CONFIG_TYPE.FOLDER) {
                const matchedParent = this.findSessionConfigParent(targetCfg, childNode);
                if (matchedParent) {
    ...
    async moveSessionConfig(sessCfg, parentId, options = {}) {
        ...
        if (!(sessCfg instanceof SessionConfig)) {
            sessCfg = (sessCfg._id && this.getSessionConfigById(sessCfg._id))
                || (sessCfg.uuid && this.getSessionConfigByUUID(sessCfg.uuid))
                || null;
        }
        ...
        const currentParent = this.findSessionConfigParent(sessCfg)
            || (sessCfg._parent instanceof SessionConfig ? sessCfg._parent : null)
            || this.sessionConfigsRoot;
        let targetParent = this.sessionConfigsRoot;

        if (parentId !== undefined && parentId !== null && parentId !== "") {
            const nextParent = this.getSessionConfigById(parentId);
            if (nextParent && nextParent.type === SESSION_CONFIG_TYPE.FOLDER) {
                targetParent = nextParent;
            }
        }

        if (!targetParent || targetParent.type !== SESSION_CONFIG_TYPE.FOLDER) {
            targetParent = this.sessionConfigsRoot;
        }

        if (currentParent._id !== targetParent._id) {
            const currentIndex = currentParent.subSessions.findIndex((item) => item._id === sessCfg._id || item.uuid === sessCfg.uuid);
            if (currentIndex >= 0) {
                currentParent.removeSubSessionConfig(currentIndex, true);
            }
            targetParent.addSessionConfig(sessCfg);
        }
    ...
    async updateSessionConfig(sessCfg, name, configParam, description = "", parentId) {
        if (!sessCfg) {
            return;
        }
        ...
        sessCfg.update(name, nextConfig, description, { publishEvent: false });
        await this.moveSessionConfig(sessCfg, parentId, { persist: false });
        await this.saveSessionConfigs();
        return true;
    }
    ...
    getSessionInstance(instance) {
        if (typeof instance === "number") {
            return this.getSessionInstanceById(instance) || this.sessionInstances[instance] || null;
        }
