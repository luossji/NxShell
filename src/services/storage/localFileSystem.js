...
class LocalFileStorage extends StorageProviderInterface {
    ...
    async saveSoftConfig(object) {
        ...
        const filePath = path.join(getSoftwareConfig(), "nxsoft.config");
        await this.service.callObject(this.handler, 'save', filePath, JSON.stringify(object));
        return filePath;
    }
    ...
    async save(name, object) {
        ...
        const filePath = path.join(this.getAppDataDirty(), STORAGE_ITEM + name);
        await this.service.callObject(this.handler, 'save', filePath, JSON.stringify(object));
        return filePath;
    }
