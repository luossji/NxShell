...
import { triggerConfigUpdateHook } from "./configUpdateHook";

const HOOK_STORAGE_NAMES = new Set(["SESSIONS"]);
...
class Storage {
    ...
    async save(name, object, sync = false) {
        ...
        const configFilePath = await ls.save(name, object)
        if (HOOK_STORAGE_NAMES.has(name)) {
            triggerConfigUpdateHook(configFilePath);
        }
        if (sync) {
