...
const TYPE_TO_SELECT_RESET_MS = 1000
...
export default {
	...
	methods: {
		...
		resetTypeToSelect() {
			if (this.typeToSelect.resetTimer) {
				clearTimeout(this.typeToSelect.resetTimer)
				this.typeToSelect.resetTimer = null
			}
			this.typeToSelect.keyword = ''
		},
		scheduleTypeToSelectReset() {
			if (this.typeToSelect.resetTimer) {
				clearTimeout(this.typeToSelect.resetTimer)
			}
			this.typeToSelect.resetTimer = setTimeout(() => {
				this.typeToSelect.keyword = ''
				this.typeToSelect.resetTimer = null
			}, TYPE_TO_SELECT_RESET_MS)
		},
		isTypeToSelectKey(evt) {
			return !evt.ctrlKey && !evt.altKey && !evt.metaKey && evt.key.length === 1 && evt.key.trim() !== ''
		},
		getSelectedAbsoluteIndex() {
			return this.selection.lastSelectItemIndex + this.view.renderStart
		},
		findTypeToSelectMatch(query, startAbsIndex) {
			if (!query || !this._items.length) {
				return -1
			}

			const normalizedQuery = query.toLocaleLowerCase()
			const total = this._items.length
			const start = ((Math.max(0, startAbsIndex) % total) + total) % total

			for (let offset = 0; offset < total; offset++) {
				const absIndex = (start + offset) % total
				const itemName = this._items[absIndex]?.value?.name?.toLocaleLowerCase?.() || ''
		...
		selectAbsoluteItem(absIndex) {
			if (absIndex < 0 || absIndex >= this._items.length) {
				return
			}

			this.getViewSize()
			if (absIndex < this.view.renderStart || absIndex >= this.view.renderEnd) {
				const halfWindow = Math.max(1, Math.floor(this.view.cols * 5))
				this.view.renderStart = Math.max(0, absIndex - halfWindow)
				this.view.renderEnd = Math.min(this._items.length, this.view.renderStart + RENDER_TOTAL)
				this.items = this._items.slice(this.view.renderStart, this.view.renderEnd)
			}

			const localIndex = absIndex - this.view.renderStart
			this.selectOneItem(localIndex)
			this.emitChange()

			this.$nextTick(() => {
				const itemHeight = this.itemHeight + this.rowSpaceSize
				const targetRow = Math.floor(localIndex / Math.max(1, this.view.cols))
				const targetY = Math.max(0, targetRow * itemHeight - itemHeight)
				this.$refs.scrollContainter?.scrollTo(0, targetY)
			})
		},
		handleTypeToSelect(evt) {
			const typedKey = evt.key.toLocaleLowerCase()
			const repeatedChar = this.typeToSelect.keyword && this.typeToSelect.keyword.split('').every((char) => char === typedKey)
			this.typeToSelect.keyword = `${this.typeToSelect.keyword}${typedKey}`
			const query = repeatedChar ? typedKey : this.typeToSelect.keyword
			const currentAbsIndex = this.getSelectedAbsoluteIndex()
			const startAbsIndex = repeatedChar ? currentAbsIndex + 1 : currentAbsIndex
			const matchedIndex = this.findTypeToSelectMatch(query, startAbsIndex)

			if (matchedIndex > -1) {
				this.selectAbsoluteItem(matchedIndex)
			}

			this.scheduleTypeToSelectReset()
		},
		...
		handleKeydown(evt) {
			if (this.isTypeToSelectKey(evt)) {
				this.handleTypeToSelect(evt)
				return
			}
