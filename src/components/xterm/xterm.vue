<template>
	<div class="pt-xterm" :style="{ 'background-color': backgroundColor }" @dragover.prevent @drop="handleFileDrop">
		<div class="xterm-search" v-if="searchShow">
			...
			<div class="search-count" v-if="searchWord">
...
export default {
	...
	methods: {
		...
		setTheme(theme = {}) {
			...
			if (this.terminal) {
				this.terminal.options.theme = this.getActiveTheme()
			}
		},

		getActiveTheme() {
			if (!(this.searchShow && this.searchWord)) {
				return { ...this.baseTheme }
			}

			return {
				...this.baseTheme,
				selectionBackground: '#ffe45e',
				selectionForeground: '#1f1600'
			}
		},

		applySearchMatchTheme() {
			if (this.terminal) {
				this.terminal.options.theme = this.getActiveTheme()
			}
		},
		...
		countMatches() {
			if (!this.terminal || !this.searchWord) {
				return 0
			}
			...
			for (let lineIndex = 0; lineIndex < logicalLines.length; lineIndex++) {
				const lineText = logicalLines[lineIndex].toLocaleLowerCase()
				let fromIndex = 0

				while (fromIndex <= lineText.length) {
					const matchedIndex = lineText.indexOf(keyword, fromIndex)
					if (matchedIndex === -1) {
						break
					}

					totalMatches++
					fromIndex = matchedIndex + Math.max(keyword.length, 1)
				}
			}

			return totalMatches
		},
		runSearch(direction = 'next', resetIndex = false) {
			if (!this.searchAddon || !this.searchWord) {
			...
			this.searchMatchCount = this.countMatches()
			if (!found || this.searchMatchCount === 0) {
				this.searchMatchIndex = 0
				this.lastSearchWord = this.searchWord
				return false
			}

			const isNewKeyword = this.lastSearchWord !== this.searchWord
			if (resetIndex || isNewKeyword || this.searchMatchIndex === 0) {
				this.searchMatchIndex = direction === 'prev' ? this.searchMatchCount : 1
			} else if (direction === 'prev') {
				this.searchMatchIndex = this.searchMatchIndex <= 1 ? this.searchMatchCount : this.searchMatchIndex - 1
			} else {
				this.searchMatchIndex = this.searchMatchIndex >= this.searchMatchCount ? 1 : this.searchMatchIndex + 1
			}
